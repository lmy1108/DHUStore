...
$(document).on("click","#saveUpdate",function () {
    ...
    /*var option = {
        url: '/demo2/admin/goods/update/'+goodsid,
        type:'post',
    };
    $("#update-goods").ajaxForm(option);*/

    $.ajax({
        url:"/demo2/admin/goods/update/",
        type:"POST",
        data:{
            goodsid:ugoodsid,
...
$(document).on("click",".templatemo-delete-btn",function () {
    ...
    swal({
        ...
        function () {
            ...
            $.ajax({
                url: "/demo2/admin/goods/delete/" + goodsid,
                type: "DELETE",
                success:function (result) {
                    swal(result.msg, "","success");
                    to_page('/demo2',currentPage);
...
function getActivity() {
    $.ajax({
        url: "/demo2/admin/activity/showjson",
        type: "post",
        success: function (result) {
            if(result.code==100) {
                $("#activity-id").empty();
...
$(document).on("click","#saveActivity",function () {
    ...
    $.ajax({
        url:"/demo2/admin/activity/update/",
        type:"POST",
        data:{
            goodsid:goodsid,
...
function build_goods_table(path,result) {
    ...
    $.each(goods, function (index,item) {
        ...
        var detailA = $("<a></a>").addClass("templatemo-link").attr("href","/demo2/detail?goodsid="+item.goodsid).append("详情");
...
function build_page_nav(path,result) {
    ...
    if(!result.info.pageInfo.hasPreviousPage) {
        ...
        prePage.click(function () {
            to_page('/demo2',result.info.pageInfo.prePage);
        });
    }
    ...
    if(!result.info.pageInfo.hasNextPage) {
        ...
        nextPage.click(function () {
            to_page('/demo2',result.info.pageInfo.nextPage);
        });
    }

    lastPage.click(function () {
        to_page('/demo2',result.info.pageInfo.lastPage);
    });
