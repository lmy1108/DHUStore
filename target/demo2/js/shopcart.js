...
function deleteCartGoods(productid) {
    $.ajax({
        url: "/demo2/deleteCart/" + productid,
        type: "DELETE",
        success: function (result) {
            // swal(result.msg, "","success");
            showcart();
...
//改变商品数量更新购物车
function updateCart(productid, newNum) {
    //获取当前数量
    // var newNum = $(".num").val();
    $.ajax({
        url: "/demo2/update",
...
function build_cart_table(result) {
    ...
    if(goods.length === 0) {
        var spareTd = $('<tr> <td colspan="6"> <div class="coupon" style="margin-left:37%;">购物车还是空的，快去<a href="/demo2/main" style="color:red;">首页</a>看看吧！ </div> </td> </tr>');
        spareTd.appendTo("#cart-table tbody");
    } else {
        $.each(goods, function (index,item) {

            var delA = $("<a></a>").addClass("delete-goods").attr("data-goodsid",item.productid).append("×");

            var deleteCart = $("<td></td>").addClass("product-remove product-remove_2")
                .append(delA);

            delA.click(function () {
                deleteCartGoods(item.productid);
            });

            var shopimage = $("<td></td>").addClass("product-thumbnail product-thumbnail-2")
                .append($("<a></a>").attr("href","/demo2/detail?productid="+item.productid)
                    .append($("<img/>").attr("src","/upload/"+item.imagepath)));
            ...
            numIput.change(function () {
               updateCart(item.productid,$(this).val());
            });
